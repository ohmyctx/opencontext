...
document.getElementById("test").addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus(resp.ok ? "Test event sent" : `Daemon returned ${resp.status}`, !resp.ok);
  } catch (err) {
...
async function checkDaemon() {
  ...
  const url = document.getElementById("daemonUrl").value || DEFAULT_SETTINGS.daemonUrl;
  try {
    const resp = await fetch(`${trimSlash(url)}/api/v1/health`, { signal: AbortSignal.timeout(3000) });
