...
metadata:
  {
    "openclaw":
      {
        "emoji": "🧠",
        "events": ["message_received", "session_start"],
...
Forwards every user message (message_received) and new session start (session_start)
to the local OpenContext daemon for activity recording and memory building.
`
...
// Forwards message_received and session_start events to the OpenContext daemon.
"use strict";
  ...
  if (event.type === "message" && event.action === "received") {
    await post({
      ...
      context: {
        ...
        sessionKey: event.context.sessionKey || event.sessionKey || "",
      },
    });
...
`, daemonAddr)
	...
	fmt.Println("OpenClaw hooks installed.")
	fmt.Printf("  hook dir:   %s\n", hookDir)
	fmt.Printf("  endpoint:   %s/api/v1/hooks/openclaw\n", daemonAddr)
	fmt.Println("\nRestart OpenClaw. User messages and session starts will be recorded.")
	fmt.Println("To verify: openclaw hooks list")
	return nil
}
...
// InstallHermes installs the OpenContext gateway hook into Hermes Agent.
// It creates ~/.hermes/hooks/opencontext/{HOOK.yaml,handler.py}.
func InstallHermes(daemonAddr string) error {
	...
	if err := os.MkdirAll(hookDir, 0o755); err != nil {
		return fmt.Errorf("create hermes hook dir: %w", err)
	}
...
`
	...
	handlerPy := fmt.Sprintf(`# OpenContext hook for Hermes Agent — auto-generated by: oc collector hermes install
# Forwards agent:start and session:start events to the OpenContext daemon.
import json
...
`, daemonAddr)
	...
	fmt.Println("Hermes Agent gateway hooks installed.")
	fmt.Printf("  hook dir:   %s\n", hookDir)
	fmt.Printf("  endpoint:   %s/api/v1/hooks/hermes\n", daemonAddr)
	fmt.Println("\nRestart the Hermes gateway (hermes gateway). User messages will be recorded.")
	return nil
}

// UninstallHermes removes the OpenContext gateway hook from Hermes Agent.
func UninstallHermes() error {
	...
	hookDir := filepath.Join(home, ".hermes", "hooks", hermesHookName)
	if err := os.RemoveAll(hookDir); err != nil {
		return fmt.Errorf("remove hermes hook dir: %w", err)
	}
	fmt.Println("Hermes Agent gateway hooks uninstalled.")
	fmt.Printf("  removed:  %s\n", hookDir)
	return nil
}
